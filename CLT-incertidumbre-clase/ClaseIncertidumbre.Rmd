...
Luego, usando la funcion `install_github()` pasando como arentos la dirección del repositorio en el cual está el paquete, y `force = TRUE` para evitar problemas durante la instalación.
...
df3$`Volumen EDTA (mL)`[2:10] <- df3$`Volumen EDTA (mL)`[2:10] + 0.5
...
curva3 <- curva3[-c(661),]
curvas <- cbind(curva1,curva2[,2]+0.5,curva3[,2]+0.2)
colnames(curvas) <- c('Volumen de EDTA (mL)', 'pCa-1','pCa-2','pCa-3')
...
for(i in 2:4){
 plot(curvasEDTA$Volumen.de.EDTA..mL.,
     curvasEDTA[,i],
     type = 'l',
...
par(mfrow=c(1,3))
for(i in 1:3){
  puntos.finales[i] <- puntoFinal(curvasEDTA$Volumen.de.EDTA..mL.,curvasEDTA[,i+1])
...
```{r}
volumenes.promedio$volumenes.promedio[1] <- mean(puntos.finales[[1]]$PE,puntos.finales[[2]]$PE,puntos.finales[[3]]$PE)


volumenes.promedio <- as.data.frame(volumenes.promedio)
volumenes.promedio$U.PF.Class[1] <- sqrt(puntos.finales[[1]]$u_PE+puntos.finales[[2]]$u_PE+puntos.finales[[3]]$u_PE)
...
par(mfrow = c(1,3))
barplot(volumenes.promedio$U.PF.Ref, main = 'u punto final')
barplot(volumenes.promedio$u_bureta, main = 'u bureta')
barplot(volumenes.promedio$u_V_EDTA, main = 'u volumen EDTA')
```
...
* Para este experimento se determino la denisdad de la cascara de huevo como: $ 1085 \dfrac{kg}{L}$
...
```{r}
volumenes.promedio$u_R <- sd.volumen/sqrt(10)
```
...
Una vez tenemos todas las fuentes de incertidumbre, podemos graficar y comparar las 
